...
to_spec4 = Spectrogram(n_fft=256, hop_length=128, power=None, window_fn = win).to(device)# , window_fn=320, hop_length=160)

def cruse_loss(spec_pred, S, l=0.3, c=0.3, reduction='mean'):
    
    # spec_pred: [batch, channel, time', frequency']
    # spec_pred[torch.abs(spec_pred)<1e-8] = 1e-8+0j
