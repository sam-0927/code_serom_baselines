...
def train_model(model, device, epochs=5, batch_size=8, learning_rate=1e-5, save_checkpoint=True, start_epoch=1):
    # 유효 파일 목록 1회 로드
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        avg_train_loss = epoch_loss / len(dataset_train)
...
def get_args():
    parser = argparse.ArgumentParser(description='Train Teacher_Pred Model')
...
if __name__ == '__main__':
    ...
    model.to(device)
    try:
        train_model(model=model, epochs=args.epochs, batch_size=args.batch_size, learning_rate=args.lr, device=device, start_epoch=start_epoch)
