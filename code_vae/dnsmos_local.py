...
def dnsmos(testset_dir='/project_ghent/ds/eva/dns_enhanced/'):
    p808_model_path = os.path.join('/project_ghent/dcase/DNSMOS', 'model_v8.onnx')
    primary_model_path = os.path.join('/project_ghent/dcase/DNSMOS', 'sig_bak_ovr.onnx')
    ...
    with ThreadPoolExecutor() as executor:
        future = executor.submit(compute_score, clips, desired_fs, is_personalized_eval)

        # 使用 tqdm 显示进度条，尽管只有一个任务
        for _ in tqdm([future], desc="Processing"):
            result = future.result()  # 获取计算结果
            # print(f"Completed: {clips}, Score: {result}")
            return (result)
