...
def run(rank, config, args):
    ...
    vocoder_config = config.get('vocoder_config', None)
    if vocoder_config:
        vocoder = WavLMDec.from_pretrained(**vocoder_config).to(args.device)
    else:
        vocoder = None
...
class Trainer:
    ...
    def _save_checkpoint(self, epoch, score):
        ...
        if score < self.best_score:
            ...
            torch.save(state_dict, os.path.join(self.checkpoint_path, 'best_model.tar'))
            print(f'[Epoch {epoch}] Best model updated: val_loss={score:.4f}')
    ...
    def _train_epoch(self, epoch):
        ...
        self.train_bar = tqdm(self.train_dataloader, ncols=80)
    ...
    def _validation_epoch(self, epoch):
        total_loss = 0
        self.validation_bar = tqdm(self.validation_dataloader, ncols=80)
        for step, (noisy, clean, info) in enumerate(self.validation_bar, 1):
            ...
            if self.vocoder and self.rank == 0 and (epoch<10 or epoch%10 == 0) and step <= 10:
    ...
    def train(self):
        ...
        if self.rank == 0:
            ...
            torch.save(self.state_dict_best,
                    ...
                    'best_model_{}.tar'.format(str(best_epoch).zfill(3))))

            print('------------Training for {} epochs has done!------------'.format(self.epochs))
            print(f'Best model: epoch={best_epoch}, val_loss={self.best_score:.4f}')
