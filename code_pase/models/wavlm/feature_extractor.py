...
class WavLM_feat(nn.Module):
    def __init__(
        ...
        frozen: bool=True,
        wavlm_cfg_path: str=None,
    ):
        super().__init__()
        self.wavlm_ckpt_path = wavlm_ckpt_path
        cpt = torch.load(self.wavlm_ckpt_path, map_location="cpu")
        if wavlm_cfg_path is not None and wavlm_cfg_path.endswith(('.yaml', '.yml')):
            with open(wavlm_cfg_path, 'r') as f:
                train_cfg = yaml.safe_load(f)
            ref_ckpt_path = train_cfg['teacher_config']['wavlm_ckpt_path']
            cfg_cpt = torch.load(ref_ckpt_path, map_location="cpu")
        else:
            cfg_cpt = torch.load(wavlm_cfg_path, map_location="cpu")
        print("[WavLM] output layer:", output_layer)

        self.cfg = WavLMConfig(cfg_cpt['cfg'])
